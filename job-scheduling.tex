...
\begin{algorithm}[H]
	\caption{Naive algorithm( under unique total ranking and low.noise assumptions )}
	\begin{algorithmic}[1]
		\Procedure{Clustering ranking }{$\mathcal{G}, \mathbf{p}, \mathbf{c}$}
		\State Derive a consistent permutation $\sigma^*$ under low.noise condition(or related conditions) \cite{Ramaswamy2013}, \cite{duchi2010}
		\State $m=\sigma^*$
		\State Let initialized $cur=r$
		\For{\texttt{$i\in[r-1]$}}
		\State
		...
		\If{$cost\leq c_{cur}$}
		\State Merge clusters $cl$ and $cl+1$. That is, set $m^{-1}(cl)\leftarrow m^{-1}(cl)\cup m^{-1}(cl+1)$; set $m^{-1}(cl+i)\leftarrow m^{-1}(cl+i+1)$ for $i\geq 1$; and set $|m|\leftarrow |m|-1$.
		\Else\quad
			break; 
...
\includegraphics[scale=0.5]{figure/eg1.png}
	\end{center}
\end{figure}
The total ranking is not unique. $\sigma_1=(1,2,3,4,5)$ or $\sigma_2=(1,2,4,3,5)$ both minimize pd loss, but will have different solution in our case. Whether $\sigma_1$ or $\sigma_2$ got outputed depends on $(\mathcal{G}, \mathbf{p})$. If we utilize surrogates from Theorem 6 from Shivani, 
\begin{equation}
\begin{split}
...
To solve this issue, we must go through all possible total ordering...
